...
- `fanOutDepsGetter` — Required for `squad_route` to spawn sessions via `spawnParallel`. Without it, `squad_route` returns `resultType: 'failure'` with `error: 'fan-out-deps-unavailable'`.
